...
#### Shipped: wall-clock-based `MAX_FRAMES` / `SKIPS`

`server.py` now stores `DEFAULT_MAX_SECONDS = 508.13` and
`DEFAULT_SKIP_SECONDS = [(44.87, 55.87)]` in wall-clock seconds and
auto-converts to frame indices at startup based on `--fps`. The 20fps
and 30fps units generated by `tools/switch_fps.sh` both now need only
`--fps` — no `--max-frames` or `--skip` overrides. Any future fps
variant (24, etc.) gets the correct visual edit for free.
